...
<table>
	...
	<tr>
		<th><%=str_writer%></th>
		<td style="text-align:center"><%=dto.getNick()%></td>
		<th><%=str_reg_date%></th>
	...
	<tr>
		<th colspan="4">
			<input class="inputbutton" type="button" value="<%=btn_modify%>"
					onclick="location='board_modifyview.do?num=<%=dto.getNum()%>&pageNum=<%=pageNum%>'">
			...
			<input class="inputbutton" type="button" value="<%=btn_reply%>"
					onclick="location='writeform.do?num=<%=dto.getNum()%>&ref=<%=dto.getRe_num()%>&re_step=<%=dto.getRe_step()%>&re_level=<%=dto.getRe_level()%>'">
